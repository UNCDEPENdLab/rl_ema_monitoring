...
##
df <- df %>% mutate(trial=y, block=output$proc_data[[params$id]]$raw_data$trials$block) %>% filter(block==i) %>% select(-block) #add a trial column, then select only trials corresponding to the block designated by params$block
