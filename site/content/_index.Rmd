...
ID_Overview$ID <- paste0("<a target='_parent' href='/",ID_Overview$ID, "/'>", ID_Overview$ID, "</a>")
#ID_Overview$ID <- convert_to_link(ID_Overview$ID)
...
for(id in ids) {
  ...
  # if the rmd for the subject does not exist
  if(file.exists(paste0('subjects/', id, '/index.Rmd')) == FALSE) {
    # then create a subject Rmd from the template
    # just makes a blank Rmd for now
    #file.create(paste0('Subjects/', id, '/_index.Rmd'))
    file.copy("../../Rmd_templates/subject/index.Rmd", paste0('subjects/', id, '/index.Rmd'))
    # blogdown::hugo_cmd("new --kind subject subjects/test/index.Rmd")
    #rmarkdown::draft(paste0("subjects/", id, "/_index.Rmd"), template="../../Rmd_templates/subject", create_dir=TRUE)
  }
}
...
widgetframe::frameWidget(datatable(ID_Overview, class='compact', escape=1, selection='none',)) # Active 
