function [edata,gap_center,std_times] = epoch2(times, data, timings, pre, post, sampling_rate)
times=int64(times);
...
gap_center = nan(size(timings,2),1);
std_times = nan(size(timings,2),1);
for i = 1:length(timings)
    ...
    if  (~isempty(ind) && gap<desiredLength)
        ...
        subsequent_samples_ind(redundant_samples_ind-1) = [];
        all_samples_ind = [ind,subsequent_samples_ind]';
        gap_center(i) = times(ind+pre_samples+1)-(timings(i));
        std_times(i) = nanstd(diff(double(times(all_samples_ind)))); %#ok<NANSTD>
    else
