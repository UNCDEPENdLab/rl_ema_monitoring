function [TFdata] = EEGanalysis_test_Validation(name,rootDir)
    ...
    % Muse raw data files:
    % rootDir->Data_Raw->subject_{name}->physio_validation ->{name}_physio_{YYYYMMDDHHmmSS}.db
    %                                  |                  | . . .
    %                                  |                  |->{name}_physio_{YYYYMMDDHHmmSS}.db
    ...
    %                                  |
    % Biosemi raw data:                ->{name}-{sessionNb}.bdf             
    ...
    for sessionNb = 1:numBiosemiSessions 
        ...
        % TFdata.biosemi{sessionNb}.data = ones(72,30,size(alignedEEG.data,2),72); % REMOVE TODO dummy 
        % TFdata.biosemi{sessionNb}.times = 2; % REMOVE
        % TFdata.biosemi{sessionNb}.freqs = 3; % REMOVE
...
function [EEG,feedbackEvents] = preprocessBiosemiData(name,rootDir,sessionNb,feedbackEvents,epochWindow)
    ...
    %% Prepare path
    % rawDir = fullfile(rootDir, 'Data_Raw', name,'Validation');
...
function museData = preprocessMuseData(name,epochWindow,rootDir)
    ...
    % pathToPhysioFiles = fullfile(rootDir,'Data_Raw',name,'physio_validation'); % Directory where to look for the raw data/physio files.
...
function data = readScheduleFile(name,rootDir)
    ...
    % filename = dir(strcat(fullfile(rootDir,'Data_Raw',name),'/*schedule.db')); 
