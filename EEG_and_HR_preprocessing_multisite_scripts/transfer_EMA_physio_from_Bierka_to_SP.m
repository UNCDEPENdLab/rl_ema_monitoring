...
for iS = 1:nS
    if regexp(subs(iS).name,'subject_\d\d\d\d\d\d')
        ...
        HR = dir('*HR.mat');
        EEG = dir('*EEG.mat');
        if length(EEG) > 0 & length(HR) > 0 & length(outcome_trials) > 0 %#ok<AND2,ISMT>
            if (~isempty(EEG.name) & ~isempty(outcome_trials.name) & ~isempty(HR.name)) %#ok<AND2>
                load(EEG.name);
                ...
                warning('No EEG file or no HR file or no outcome_trials file %s',subs(iS).name);
            end
        end
    end
    cd(data_in);
