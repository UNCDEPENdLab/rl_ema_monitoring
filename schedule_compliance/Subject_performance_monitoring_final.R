...
data = dbConnect(SQLite(), "B102_schedule (1).db")
...
#test what percentage of blocks (after the practice session) have relative accuracy of less than 70% (with feedback)
...
percentage_of_poor_blocks=length(blocks_with_poor_performance$poor_blocks)/(max(relative_accuracy_by_block$block)-5)
...
if (nrow(blocks_with_poor_performance)>0){
...
if (nrow(blocks_with_poor_performance)>0){
  figure_5=figure_5+geom_point(data=blocks_with_poor_performance, aes(x=poor_blocks, y=0.5), color="darkorchid1", size=2.2)}
