...
cat('\n')
cat("<!-- This is a comment for subject", iS, "-->\n")
ggplot(df, aes(x=t0,y=mean)) + geom_line() + geom_errorbar(aes(ymin=mean-st_err,ymax=mean+st_err),width=0.2) + scale_x_continuous(breaks=c(0,a2f,max(df$t0)),labels=c(-1000,0,100000),name='time [ms]') +
  geom_vline(xintercept = a2f, lty = "dashed", color = "#FF0000", size = 2)
cat('\n')
```