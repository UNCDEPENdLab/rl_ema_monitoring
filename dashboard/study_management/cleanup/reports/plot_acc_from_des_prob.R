...
fig_name = paste0("acc_from_des_prob.png")
try({
  acc_from_des_prob_graph <- ggplot(data=objective_accuracy_by_block, aes(x=block, y=mean*100, group=feedback, colour=feedback)) +
    geom_line() +
    ...
    ylab("percent correct choice")+
    ggtitle("Accuracy according to designated probabilities") +
    labs(subtitle=paste0("Subject ID:",subj)) +
  ...
  dev.off() # 2023-03-21 AndyP moved dev.off() inside try({})
})
