...
# output$proc_data[[params$id]]$raw_data$trials <- output$proc_data[[params$id]]$raw_data$trials[1:756,] # this doesn't work AndyP 2021-05-24
...
dq <- dq %>% mutate(trial=trials, blocks=blocks) %>% filter(blocks==params$block)  #add a trial column, then select only trials corresponding to the block designated by params$block, Fixed AndyP 2021-05-24
